#!/usr/bin/env python3
"""
test_trellis.py — testa o worker TRELLIS_ONLY (Fase 4): envia uma imagem local
e salva o .glb resultante.

A Fase 4 recebe imagem e devolve .glb (sem prompt de texto), então usamos este
cliente dedicado em vez do client.py oficial.

Uso:
  python test_trellis.py <ENDPOINT_ID> saidas/gato.png --name gato
...
def main() -> None:
    ...
    ap.add_argument("image", help="caminho da imagem de entrada (PNG/JPG)")
    ap.add_argument("--name", default="model")
    ap.add_argument("--out", default="./saidas")
    ap.add_argument("--texture-size", type=int, default=None)
    ap.add_argument("--decimation-target", type=int, default=None)
    args = ap.parse_args()
    ...
    inp = {"image_base64": b64}
    if args.texture_size:
        inp["texture_size"] = args.texture_size
    if args.decimation_target:
        inp["decimation_target"] = args.decimation_target

    base = f"https://api.runpod.ai/v2/{args.endpoint_id}"
    headers = {"Authorization": f"Bearer {API_KEY}", "Content-Type": "application/json"}

    print(f"[teste] enviando imagem: {img_path} ({img_path.stat().st_size//1024} KB)")
    t0 = time.monotonic()
